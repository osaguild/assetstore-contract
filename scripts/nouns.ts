...
const { nounsTokenContract, nounsDescriptorContract, nounsSeederContract } =
  getContractsForChainOrThrow(chainId, provider);

async function main() {
  console.log("nounsToken", nounsTokenContract.address);
  const [backgroundCount] =
    await nounsDescriptorContract.functions.backgroundCount();
  const [bodyCount] = await nounsDescriptorContract.functions.bodyCount();
  const [accessoryCount] =
    await nounsDescriptorContract.functions.accessoryCount();
