...
export const deploy: any = async (setWhitelist = true) => {
  ...
  const materialTokenStoreFactory = await ethers.getContractFactory(
    "MaterialToken"
  );
  const materialToken = await materialTokenStoreFactory.deploy(
    assetStore.address,
    assetStore.address,
    developer,
    proxy
  ...
  const args =
    `const assetStore = "${assetStore.address}";\n` +
  ...
  const verifyScript =
    `npx hardhat verify ${assetStore.address} --network ${network.name}\n` +
