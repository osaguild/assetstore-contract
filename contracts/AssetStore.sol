...
abstract contract AssetStoreCore is Ownable, IAssetStoreRegistry {
    ...
    function _getCategoryId(
        ...
        (uint32 id, bool created) = categorySet.getOrCreateId(
            category,
            validator
        );
        if (created) {
    ...
    modifier validateAsset(AssetInfo memory _assetInfo) {
        uint256 size = _assetInfo.parts.length;
        uint256 i;
        for (i = 0; i < size; i++) {
            Part memory part = _assetInfo.parts[i];
            require(
                validator.validate(bytes(part.color)),
                "Invalid AssetData Color"
            );
        }
        require(
            validator.validate(bytes(_assetInfo.name)),
            "Invalid AssetData Name"
        );
        _;
    }
     ...
     */
    function _registerAsset(AssetInfo memory _assetInfo)
        internal
        validateAsset(_assetInfo)
        returns (uint256)
    {
        uint32 groupId = _getGroupId(_assetInfo.group);
        uint32 categoryId = _getCategoryId(
            _assetInfo.group,
            groupId,
            _assetInfo.category
        );
        ...
        if (minterLength > 0) {
            require(
                minterLength <= 32,
                "AssetSgore: _registerAsset, minter name is too long."
            );
        ...
        require(
            assetCatalog.assetNameToId[_assetInfo.name] == 0,
            "Asset already exists with the same group, category and name"
        );
    ...
    modifier assetExists(uint256 _assetId) {
        require(
            _assetId > 0 && _assetId < nextAssetIndex,
            "AssetStore: assetId is out of range"
        );
        _;
    }
    ...
    modifier partExists(uint256 _partId) {
        require(
            _partId > 0 && _partId < nextPartIndex,
            "partId is out of range"
        );
        _;
    }
    ...
    function _getPart(uint256 _partId)
        internal
        view
        partExists(_partId)
        returns (Part memory)
...
abstract contract AssetStoreAdmin is AssetStoreCore {
    ...
    modifier onlyAdmin() {
        require(
            ...
            "AssetStoreAdmin: caller is not the admin"
        );
        _;
    }
    ...
    function setWhitelistStatus(address _address, bool _status)
        external
        onlyAdmin
    {
    ...
    function setDisabled(uint256 _assetId, bool _status)
        external
        assetExists(_assetId)
        onlyAdmin
    {
    ...
    function getRawAsset(uint256 _assetId)
        external
        view
        onlyAdmin
        returns (Asset memory)
    ...
    function getRawPart(uint256 _partId)
        external
        view
        onlyAdmin
        returns (Part memory)
...
contract AppStoreRegistory is AssetStoreAdmin {
    modifier onlyWhitelist() {
        require(
            disableWhitelist || whitelist[msg.sender],
            "AssetStore: The sender must be in the white list."
        );
        _;
    }

    function registerAsset(AssetInfo memory _assetInfo)
        external
        override
        onlyWhitelist
        returns (uint256)
    ...
    function registerAssets(AssetInfo[] memory _assetInfos)
        external
        override
        onlyWhitelist
    {
...
contract AssetStore is AppStoreRegistory, IAssetStore {
    ...
    modifier enabled(uint256 _assetId) {
        require(
            disabled[_assetId] != true,
            "AssetStore: this asset is diabled"
        );
        _;
    }
    ...
    function getAssetCountInCategory(
        ...
        StringSet.Set storage categorySet = categorySets[groupId];
        return
            assetCatalogs[groupId][categorySet.getId(_category)].nextAssetIndex;
    }
    ...
    function getAssetIdInCategory(
        ...
        uint32 groupId = groupSet.getId(_group);
        StringSet.Set storage categorySet = categorySets[groupId];
        AssetCatalog storage assetCatalog = assetCatalogs[groupId][
            categorySet.getId(_category)
        ];
        require(
            _assetIndex < assetCatalog.nextAssetIndex,
            "The assetIndex is out of range"
        );
        return assetCatalog.assetIds[_assetIndex];
    }

    // Returns the assetId of the specified group/category/name.
    function getAssetIdWithName(
        string memory _group,
        string memory _category,
        string memory _name
    ) external view override returns (uint256) {
        ...
        StringSet.Set storage categorySet = categorySets[groupId];
        return
            assetCatalogs[groupId][categorySet.getId(_category)].assetNameToId[
                _name
            ];
    }

    function _getDescription(Asset memory asset)
        internal
        view
        returns (bytes memory)
    {
        ...
        return
            abi.encodePacked(
                group,
                "/",
                categorySets[asset.groupId].nameAtIndex(asset.categoryId - 1),
                "/",
                asset.name
            );
    }
     ...
     */
    function _tagForAsset(uint256 _assetId)
        internal
        pure
        returns (string memory)
    {
    ...
    function _safeGenerateSVGPart(uint256 _assetId, string memory _tag)
        internal
        view
        returns (bytes memory)
    {
    ...
    function generateSVGPart(uint256 _assetId, string memory _tag)
        external
        view
        override
        enabled(_assetId)
    ...
    function generateSVG(uint256 _assetId)
        external
        view
        override
        enabled(_assetId)
    ...
    function getAttributes(uint256 _assetId)
        external
        view
        override
        returns (AssetAttributes memory)
    {
        Asset memory asset = _getAsset(_assetId);
        ...
        attr.category = categorySets[asset.groupId].nameAtIndex(
            asset.categoryId - 1
        );
        attr.width = 1024;
        attr.height = 1024;
