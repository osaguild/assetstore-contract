...
abstract contract OnChainCanvasCore is ERC721A {
    ...
    string public description =
        "This is one of effts to create (On-Chain Asset Store)[https://assetstore.wtf/draw]. ";
    ...
    constructor(
        ...
        AssetComposer composer = new AssetComposer(
            _assetStore,
            providerRegistry
        );
...
contract OnChainCanvas is OnChainCanvasAdmin, IAssetStoreToken {
    ...
    function mintWithAsset(
        IAssetStoreRegistry.AssetInfo memory _assetInfo,
        uint256 _affiliate,
        uint256 _strokeWidth,
        RemixInfo[] memory _remixes,
        IAssetComposer.AssetLayer[] memory _overlays
    ) external payable {
        uint256 tokenId = _nextTokenId();
        uint256 nextAssetId = assetStore.getAssetCount() + 1; // 1-based
        _assetInfo.group = string(
            abi.encodePacked("Draw Your Own ", _assetInfo.group)
        );
        ...
        // @notice
        if (
            _remixes.length == 0 && _overlays.length == 0 && _strokeWidth == 0
        ) {
            ...
            IAssetComposer.AssetLayer[]
                memory layers = new IAssetComposer.AssetLayer[](
                    offset + 1 + _overlays.length
                );
            ...
            assetComposer.processPayoutWithSkipIndex{
                value: (msg.value * 975) / 1000
            }(compositionId, offset);
        ...
        if (
            _affiliate > 0 &&
            _isPrimary(_affiliate) &&
            ownerOf(_affiliate) != msg.sender
        ) {
    ...
    function generateSVG(
        string memory _svgPart,
        uint256 _style,
        string memory _tag
    ) public pure override returns (string memory) {
        // Constants of non-value type not yet implemented by Solidity
        string[4] memory backColors = ["", "url(#silver)", "url(#gold)", ""];

        uint256 index = _style % _tokensPerAsset;
        bytes memory image = abi.encodePacked(SVGHeader, _svgPart);
        if (index == 1) {
        ...
        if (index == 3) {
            image = abi.encodePacked(
                image,
                ' <use filter="url(#f1)" href="#',
                _tag,
                '" />\n'
            );
        } else {
            image = abi.encodePacked(image, ' <use href="#', _tag, '" />\n');
        }
     ...
     */
    function assetIdOfToken(uint256 _tokenId)
        public
        view
        override
        returns (uint256)
    {
        require(
            _exists(_tokenId),
            "CustomToken.assetIdOfToken: nonexistent token"
        );
    ...
    function _generateTraits(
        uint256 _tokenId,
        IAssetStore.AssetAttributes memory _attr
    ) internal view returns (bytes memory) {
        bytes memory pack = abi.encodePacked(
            "{"
            '"trait_type":"Primary",'
            '"value":"',
            _isPrimary(_tokenId) ? "Yes" : "No",
            '"'
            "},{"
            '"trait_type":"Group",'
            '"value":"',
        ...
        pack = abi.encodePacked(
            ...
            '"trait_type":"Minter",'
            '"value":"',
            (bytes(_attr.minter).length > 0)
                ? assetStore.getStringValidator().sanitizeJason(_attr.minter)
                : bytes("(anonymous)"),
            '"'
            "},{"
            '"trait_type":"Remix",'
            '"value":"',
            remixBase[_tokenId] > 0
                ? (remixBase[_tokenId] - 1).toString()
                : "(none)",
            '"'
            "}"
        );
    ...
    function generateSVGPart(uint256 _tokenId)
        public
        view
        returns (string memory, string memory)
    {
        ...
        IAssetStore.AssetAttributes memory attr = assetStore.getAttributes(
            assetId / 2
        );
        string memory svgPart = assetStore.generateSVGPart(
            assetId / 2,
            attr.tag
        );
        ...
        returns (string memory)
    {
        ...
        bytes memory image = bytes(
            generateSVG(svgPart, _tokenId % _tokensPerAsset, tag)
        );
