...
contract SplatterArtProvider is IAssetProvider, IERC165, Ownable {
    ...
    uint256 constant stylesPerSeed = 4;
    ...
    function getColorScheme(Randomizer.Seed memory _seed)
        internal
        pure
        returns (Randomizer.Seed memory seed, string[] memory scheme)
    {
    ...
    {
        Randomizer.Seed memory seed = Randomizer.Seed(
            _assetId / stylesPerSeed,
            0
        );
        ...
        if (_assetId % stylesPerSeed == 0) {
            ...
            for (uint256 i = 0; i < scheme.length * 10; i++) {
                (seed, path) = splatter.generatePath(seed, props);
                body = abi.encodePacked(
                    body,
                    '<path d="',
                ...
                size += 100;
                uint256 margin = (1024 - (1024 * size) / 1000) / 2;
                uint256 x;
                uint256 y;
