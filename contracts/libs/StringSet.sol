...
library StringSet {
    ...
    function getOrCreateId(
        ...
        require(
            _validator.validate(bytes(_name)),
            "StringSet.getId: Invalid String"
        );
    ...
    function getId(Set storage _set, string memory _name)
        internal
        view
        returns (uint32)
    {
     ...
     */
    function nameAtIndex(Set storage _set, uint32 _index)
        internal
        view
        returns (string memory)
    {
        require(
            _index < _set.nextIndex,
            "StringSet.nameAtIndex: The index is out of range"
        );
