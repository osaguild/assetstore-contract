...
library Trigonometry {
    ...
    function sinQuarter(uint256 _angle) internal pure returns (int256) {
        if (_angle == 0x1000) {
            return 0x7fff;
        }
        uint256 index = _angle / 0x100; // high 4-bit
        uint256 interp = _angle & 0xFF; // low 8-bit
        uint256 x1 = sin_table_lookup(index);
        uint256 x2 = sin_table_lookup(index + 1);
        return int256(x1 + ((x2 - x1) * interp) / 0x100);
    }
