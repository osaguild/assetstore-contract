...
contract AssetProviderRegistry is IAssetProviderRegistry, Ownable {
    ...
    function registerProvider(IAssetProvider _provider)
        external
        override
        returns (uint256 providerId)
    {
        IAssetProvider.ProviderInfo memory providerInfo = _provider
            .getProviderInfo();
        ...
        if (providerId != 0) {
            ...
            require(
                _provider.getOwner() == existingProvider.getOwner(),
                "AssetComposer:registerProvider, already registered"
            );
            ...
            // New Registration
            require(
                _provider == providerInfo.provider,
                "AssetComposer:registerProvider, address mismatch"
            );
        ...
        returns (uint256)
    {
        ...
        require(
            ...
            string(
                abi.encodePacked(
                    "AssestComposer:getProviderId, the provider does not exist:",
                    _key
                )
            )
        );
    ...
    function setDisabledProvider(uint256 _providerId, bool _status)
        external
        onlyAdmin
    {
