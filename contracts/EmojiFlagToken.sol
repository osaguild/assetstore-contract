...
contract EmojiFlagToken is Ownable, ERC721A, IAssetStoreToken {
    ...
    string public description =
        "This is one of effts to create (On-Chain Asset Store)[https://assetstore.wtf]. "
        ...
        returns (uint256)
    {
        require(
            _exists(_tokenId),
            "EmojiFlagToken.assetIdOfToken: nonexistent token"
        );
        ...
        return
            abi.encodePacked(
                ...
                (bytes(_attr.minter).length > 0)
                    ? assetStore.getStringValidator().sanitizeJason(
                        _attr.minter
                    )
                    : bytes("(anonymous)"),
                '"'
        ...
        returns (string memory)
    {
        require(
            _exists(_tokenId),
            "EmojiFlagToken.tokenURI: nonexistent token"
        );
        ...
        bytes memory image = bytes(
            generateSVG(svgPart, _tokenId % _tokensPerAsset, attr.tag)
        );
