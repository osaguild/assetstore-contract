...
abstract contract AssetComposerAdmin is Ownable {
    ...
    modifier onlyAdmin() {
        require(
            ...
            "AssetComposer: caller is not the admin"
        );
        _;
    }
    ...
    function setDisabledComposition(uint256 _compositionId, bool _status)
        external
        onlyAdmin
    {
...
contract AssetComposerCore is AssetComposerAdmin, IAssetComposer {
     ...
     */
    function registerComposition(AssetLayer[] memory _layers)
        external
        override
        returns (uint256 compositionId)
    {
        ...
        for (uint256 i = 0; i < _layers.length; i++) {
            ...
            assets[compositionId][i] = ProviderAsset(
                uint128(providerId),
                uint128(assetId)
            );
            ...
            if (transform.length > 0) {
                require(
                    validator.validate(transform),
                    "register: Invalid transform"
                );
...
contract AssetComposer is AssetComposerCore, IAssetProvider, IERC165 {
     ...
     */
    function generateSVGPart(uint256 _compositionId)
        public
        view
        override
        returns (string memory, string memory)
    {
        ...
        if (!disabledComposition[_compositionId]) {
            for (uint256 i = 0; i < layerLength; i++) {
                ...
                (ProviderInfo memory info, bool disabled) = registry
                    .getProvider(uint256(asset.providerId));
                if (disabled) {
                    continue;
                }
                (svgPart, tagId) = info.provider.generateSVGPart(
                ...
                }
                {
                    ...
                    if (stroke > 0) {
                        uses = abi.encodePacked(
                            uses,
                            ' stroke="black" stroke-linecap="round" stroke-width="',
                            uint8(stroke).toString(),
                            'px"'
                        );
                    }
                }
        ...
        tagId = string(
            abi.encodePacked(providerKey, _compositionId.toString())
        );
        svgPart = string(
            abi.encodePacked(defs, '<g id="', tagId, '" >\n', uses, "</g>\n")
        );
     ...
     */
    function processPayoutWithSkipIndex(
        ...
        for (uint256 i = 0; i < layerLength; i++) {
            ...
            ProviderAsset memory asset = assets[_compositionId][i];
            (ProviderInfo memory info, ) = registry.getProvider(
                uint256(asset.providerId)
            );
