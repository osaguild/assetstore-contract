...
badAssetColor.parts = [
  {
    color: "#fff~",
    body: badAssetColor.parts[0].body,
  },
];
...
describe("AssetStore Component Test", function () {
  ...
  it("Register 'Done'", async function () {
    ...
    expect(await contract.getCategoryNameAtIndex(asset.group, 0)).equal(
      asset.category
    );
    expect(
      await contract.getAssetCountInCategory(asset.group, asset.category)
    ).equal(1);
    expect(
      await contract.getAssetIdInCategory(asset.group, asset.category, 0)
    ).equal(assetId);
    expect(
      await contract.getAssetIdWithName(asset.group, asset.category, asset.name)
    ).equal(assetId);
    const attr: any = await contract.getAttributes(assetId);
    expect(
      attr.name == asset.name &&
        attr.group == asset.group &&
        attr.category == asset.category
    ).equal(true);
    expect(
      attr.minter == asset.minter && attr.soulbound == asset.soulbound
    ).equal(true);
  ...
  it("Register 'Settings'", async function () {
    ...
    expect(
      await contract.getAssetIdInCategory(asset.group, asset.category, 1)
    ).equal(assetId);
    expect(
      await contract.getAssetIdWithName(asset.group, asset.category, asset.name)
    ).equal(assetId);
  ...
  it("Register 'Home'", async function () {
    ...
    expect(await contract.getCategoryNameAtIndex(asset.group, 1)).equal(
      asset.category
    );
    expect(
      await contract.getAssetCountInCategory(asset.group, asset.category)
