...
describe("MaterialToken minting test", function () {
  ...
  it("First mint", async function () {
    ...
    expect(await materialToken.balanceOf(user0.address)).equal(
      tokensPerAsset - 1
    );
  ...
  it("Affiliated mint", async function () {
    ...
    expect(await materialToken.balanceOf(user1.address)).equal(
      tokensPerAsset - 1
    );
    ...
    expect(await materialToken.balanceOf(user2.address)).equal(
      tokensPerAsset - 1
    );
  ...
  it("Duplicated assets", async function () {
    ...
    expect(
      await catchError(async () => {
        await materialToken.mintWithAsset(assetSettings, 0);
      })
    ).equal(true);
    expect(
      await catchError(async () => {
        await materialToken.mintWithAsset(assetAccount, 0);
      })
    ).equal(true);
  });
  ...
  it("Verify onlyOwner security", async function () {
    ...
    expect(
      await catchError(async () => {
        await assetStore1.setWhitelistStatus(materialToken.address, false);
      })
    ).equal(true);
    expect(
      await catchError(async () => {
        await assetStore1.setDisableWhitelist(true);
      })
    ).equal(true);
    expect(
      await catchError(async () => {
        await assetStore1.setDisabled(1, true);
      })
    ).equal(true);
    expect(
      await catchError(async () => {
        await assetStore1.getRawAsset(1);
      })
    ).equal(true);
    expect(
      await catchError(async () => {
        await assetStore1.getRawPart(1);
      })
    ).equal(true);
  });
